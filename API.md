...
**Validation:** `date` must be a real ISO `YYYY-MM-DD` value no later than the current Asia/Karachi local date. Future dates return `400 DATE_IN_FUTURE`.
**Status:** implemented
...
**Notes:** Returns up to 200 most recent audit entries with editor name/role and old/new status. An optional `date` filter must be a real ISO `YYYY-MM-DD` value no later than the current Asia/Karachi local date.
**Status:** implemented
...
**Query params:** `classSectionId` (required), `date` (required)
**Notes:** CSV includes school name, address, phone, generated by, role, and timestamp as header rows. `date` must be a real ISO `YYYY-MM-DD` value no later than the current Asia/Karachi local date; future or malformed values are rejected server-side.
**Status:** implemented
...
**Validation:** Supplied `from` and `to` dates must be real ISO `YYYY-MM-DD` values no later than the current Asia/Karachi local date, and `from` cannot be after `to`.
